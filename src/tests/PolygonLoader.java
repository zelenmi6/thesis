...
public class PolygonLoader {
	...
	public void loadRectangles() {
		try {
			...
			dao.addPicture(dataSetId, "noPath", generatePolygon(0, 0, 7, 0, 7, 7, 0, 7));
			dao.addPicture(dataSetId, "noPath2", generatePolygon(0, 0, 3.5, 0, 3.5, 15, 0, 15));
			dao.addPicture(dataSetId, "noPath3", generatePolygon(10, 0, 18.94, 4.47, 8.94, 24.47, 0, 20));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void loadTrapezoids() {
		try {
			...
			dataSetId = dao.addDataSet(areaId, "noPath", 0, 0);
			loadManyRandom();
			dao.addPicture(dataSetId, "noPath", generatePolygon(0, 0, 10, 0, 9, 10, 3, 10));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private void loadManyRandom() {
		...
		for (int i = 0; i < 10000000; i ++) {
			...
			int y4 = y3;
			try {
				dao.addPicture(dataSetId, "noPath/" + i, generatePolygon(x1, y1, x2, y2, x3, y3, x4, y4));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	private Vector3d [] generatePolygon(double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4) {
